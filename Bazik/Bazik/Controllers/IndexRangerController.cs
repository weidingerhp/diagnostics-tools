...
namespace Bazik.Controllers
{
    public class IndexRangerController : Controller
    {
        ...
        public ActionResult ShowDashboard(String serverName = "localhost")
        {
            var result = new MsSqlDashboardModel { ServerName = serverName};

            using (var conn = new SqlConnection(WebConfigurationManager.ConnectionStrings["MsSqlConnString#" + serverName].ConnectionString))
            {
                conn.Open();
...
order by dt.database_transaction_log_bytes_used desc");
                ...
                var sessionSet = new HashSet<MsSqlSession>(conn.Query<MsSqlSession>(
@"select top (@sessionCnt) session_id, login_time,
        (select count(1) from sys.dm_exec_requests where session_id = s.session_id) as request_num,
    ...
    from sys.dm_exec_sessions s where
        is_user_process = 1 and (reads > 10 or writes > 10) and session_id <> @@spid
    order by (reads + writes) desc, cpu_time desc", new { sessionCnt = Int32.Parse(WebConfigurationManager.AppSettings["ActiveSessionsNum"] ?? "20") }), new MsSqlSesssionEqualityComparer());

                var blockingSessions = result.ActiveRequests.Select(r => r.blocking_session_id).Distinct();
                if (blockingSessions.Any())
                {
                    sessionSet.UnionWith(conn.Query<MsSqlSession>(
        ...
        status, cpu_time, memory_usage,
        reads, logical_reads, writes, transaction_isolation_level
    from sys.dm_exec_sessions s where session_id in @blockingSessions",
                        new { blockingSessions }));
                }
                result.ActiveSesssions = sessionSet;
            }
        ...
        public ActionResult Session(String srv, short sid)
        {
            ViewBag.srv = srv;
...
from sys.dm_exec_connections c
    ...
    where c.session_id in (@sid)
        ...
        public ActionResult Request(String srv, short sid, short rid)
        {
            ViewBag.srv = srv;
            using (var conn = new SqlConnection(WebConfigurationManager.ConnectionStrings["MsSqlConnString#" + srv].ConnectionString))
            {
